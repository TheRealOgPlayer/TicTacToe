...
namespace TicTacToe
{
    class Program
    {
        ...
        static void botCheck()
        {
            ...
            if (diffecultyChoice == "Hard" || diffecultyChoice == "Impossable")
            {
                //Checks to see if player is about to win
                if (diffecultyChoice == "Impossable")
                {
                    possibleWinStates = "H036H147H258H048H246H642H840H360H471H582H012H345H678H786H453H120H840H174H354H574H374H154H134";
                }
                else
                {
        ...
        static void handleMoves()
        {
            if (playersTurn && canRun)
            {
                ...
                {
                    if (!debuggingMode) Console.Clear();
                    if (!debuggingMode) Console.Write("The cords you have entered are not possible try another set of cords.\nWhen ready press any key.");
                    if (debuggingMode) Console.Write("The cords you have entered are not possible try another set of cords.\n\nDebugging Info:\n\n{0}\n\nWhen ready press any key.", e);
                    Console.ReadKey();
                    printBoard(false);
                }
            }
        ...
        static void StartGame()
        {
            try
            {
                ...
                Console.WriteLine("What Diffiulty Do You Want(Easy(E) Medium(M) Hard(H)) Impossable(I) Enable/Disable Debuggin(D) Exit(999)");
            ...
            {
                if (!debuggingMode) Console.Clear();
                if (!debuggingMode) Console.WriteLine("How did you get here? Error of some kind.\nWhen ready press any key.");
                if (debuggingMode) Console.WriteLine("How did you get here? Error of some kind.\n\nDebugging Info:\n\n{0}\n\nWhen ready press any key.", e);
                Console.ReadKey();
                StartGame();
            }
